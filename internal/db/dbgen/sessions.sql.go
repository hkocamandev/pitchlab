...
const getSessionLiveMetrics = `-- name: GetSessionLiveMetrics :one
SELECT
...
type GetSessionLiveMetricsRow struct {
	PitchCount         int64
	WhiffCount         int64
...
func (q *Queries) GetSessionLiveMetrics(ctx context.Context, sessionID uuid.UUID) (GetSessionLiveMetricsRow, error) {
	row := q.db.QueryRow(ctx, getSessionLiveMetrics, sessionID)
	var i GetSessionLiveMetricsRow
	err := row.Scan(
		&i.PitchCount,
		&i.WhiffCount,
		&i.AvgReleaseSpeed,
